# ObsidianUI — Grid Lift

[Canonical page](https://www.obsidianui.dev/docs/grid-lift) · [Agent guide](https://www.obsidianui.dev/agent-instructions.md)

A fine grid lifts into a dimensional text or SVG mask around your pointer.

## Preview

[Open the interactive component preview](https://www.obsidianui.dev/docs/grid-lift)

Move across the grid to reveal the raised wordmark.
...
```bash
npx shadcn@latest add "https://www.obsidianui.dev/r/grid-lift.json"
```
...
Download the complete manifest: [grid-lift.json](https://www.obsidianui.dev/r/grid-lift.json). It includes every required local file and package dependency.
...
### components/block/grid-lift.jsx

Installation target: `@components/block/grid-lift.jsx`
...
export function GridLift({ text = "OBSIDIANUI", imageSrc = "https://cdn-athrix.milliondollarinternet.lol/effects/grid-lift/obsidianui-wordmark.svg", showControls = false, className, style } = {}) {
...
Move across the grid to reveal the raised wordmark. The effect stays inside its container and respects reduced motion preferences.
