...
export const MobileNotification = () => {
    ...
    useEffect(() => {
        ...
        gsap.fromTo(
            notification,
            { x: reduceMotion ? 0 : "120%", opacity: 0, scale: reduceMotion ? 1 : 0.95 },
            { x: "0%", opacity: 1, scale: 1, duration: reduceMotion ? 0 : 0.8, ease: "power3.out" }
        );
    ...
    }, [isVisible, reduceMotion, dismissNotification]);
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
        if (touchStartRef.current === null || !notificationRef.current) return;
    ...
    const handleTouchEnd = () => {
        ...
        if (Math.abs(swipeDiff) > 100) {
            ...
            if (notificationRef.current) {
                gsap.to(notificationRef.current, { x: 0, duration: reduceMotion ? 0 : 0.4, ease: "power2.out" });
            }
            // Restart 10s timer
            timerIdRef.current = window.setTimeout(() => {
                dismissNotification('right');
    ...
    return (
        ...
        >
            <div className="relative flex items-center gap-3 pl-3 pr-2 py-3 rounded-[24px] bg-white/80 dark:bg-black/80 backdrop-blur-xl border border-neutral-200/50 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
                ...
                {/* Message */}
                <p className="landing-copy text-xs font-medium leading-relaxed pr-2">
