...
export const scrollGalleryImages = [1, 2, 3, 4].map(index => `/effects/parallax-gallery/parallax-gallery-img${String(index).padStart(2, "0")}.jpg`);
export const scrollMarqueeImages = [1, 2, 3, 4].map(index => ({
  id: index,
  src: `/effects/draggable-marquee/draggable-marquee-img${String(index).padStart(2, "0")}.jpg`,
...
export const svgMarqueeImages = Array.from({ length: 8 }, (_, index) => `/effects/svg-path-marquee/svg-path-marquee-img${String(index + 1).padStart(2, "0")}.png`);
