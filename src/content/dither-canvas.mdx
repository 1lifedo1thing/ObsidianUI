...
<ComponentPreview component={<EffectPreview slug="dither-canvas" />} code={effectExamples["dither-canvas"]} description="Move across the canvas to disturb the texture." previewClassName="p-0 sm:p-0" />

## Install using CLI

<CLICommand componentName="dither-canvas" />
...
<Dependencies step={2} title="Copy the source code" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">components/block/dither-canvas.jsx</code></p>
