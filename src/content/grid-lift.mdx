...
<ComponentPreview component={<EffectPreview slug="grid-lift" />} code={effectExamples["grid-lift"]} description="Move across the grid to reveal the raised wordmark." previewClassName="p-0 sm:p-0" />

## Install using CLI

<CLICommand componentName="grid-lift" />
...
<Dependencies step={2} title="Copy the source code" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">components/block/grid-lift.jsx</code></p>
...
<Dependencies step={5} title="Add the supporting file" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">public/effects/grid-lift/obsidianui-wordmark.svg</code></p>
