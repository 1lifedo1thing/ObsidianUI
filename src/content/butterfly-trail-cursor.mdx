...
<ComponentPreview component={<EffectPreview slug="butterfly-trail-cursor" />} code={effectExamples["butterfly-trail-cursor"]} description="Move the pointer across the preview to release butterflies." previewClassName="p-0 sm:p-0" />

## Install using CLI

<CLICommand componentName="butterfly-trail-cursor" />
...
<Dependencies step={1} title="Install dependencies">
  <CodeBlock code="npm install @react-three/drei @react-three/fiber clsx motion tailwind-merge three" />
</Dependencies>

<Dependencies step={2} title="Copy the source code" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">components/block/butterfly-trail-cursor.jsx</code></p>
