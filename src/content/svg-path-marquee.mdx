...
<ComponentPreview component={<EffectPreview slug="svg-path-marquee" />} code={effectExamples["svg-path-marquee"]} description="Drag the images or use the arrow keys while the path is focused." previewClassName="p-0 sm:p-0" />

## Install using CLI

<CLICommand componentName="svg-path-marquee" />
...
<Dependencies step={2} title="Copy the source code" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">components/block/svg-path-marquee.jsx</code></p>
