...
<ComponentPreview component={<EffectPreview slug="interactive-hover-slider" />} code={effectExamples["interactive-hover-slider"]} description="Hover or focus a project row to reveal its image." previewClassName="p-0 sm:p-0" />

## Install using CLI

<CLICommand componentName="interactive-hover-slider" />
...
<Dependencies step={1} title="Install dependencies">
  <CodeBlock code="npm install clsx gsap tailwind-merge three" />
</Dependencies>

<Dependencies step={2} title="Copy the source code" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">components/block/interactive-hover-slider.jsx</code></p>
