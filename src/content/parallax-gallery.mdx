...
<ComponentPreview component={<EffectPreview slug="parallax-gallery" />} code={effectExamples["parallax-gallery"]} description="Scroll inside the preview to move through the photographs." previewClassName="p-0 sm:p-0" />

## Install using CLI

<CLICommand componentName="parallax-gallery" />
...
<Dependencies step={1} title="Install dependencies">
  <CodeBlock code="npm install gsap motion" />
</Dependencies>

<Dependencies step={2} title="Copy the source code" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">components/block/parallax-gallery.jsx</code></p>
