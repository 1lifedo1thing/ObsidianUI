...
<ComponentPreview component={<EffectPreview slug="curved-plane" />} code={effectExamples["curved-plane"]} description="Drag the images or scroll over the canvas." previewClassName="p-0 sm:p-0" />

## Install using CLI

<CLICommand componentName="curved-plane" />
...
<Dependencies step={2} title="Copy the source code" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">components/block/curved-plane.jsx</code></p>
