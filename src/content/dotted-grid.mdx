...
<ComponentPreview component={<EffectPreview slug="dotted-grid" />} code={effectExamples["dotted-grid"]} description="Move across the grid to draw a trail." previewClassName="p-0 sm:p-0" />

## Install using CLI

<CLICommand componentName="dotted-grid" />
...
<Dependencies step={2} title="Copy the source code" className="docs-source-card">
  <p className="mb-4 text-sm text-muted-foreground">Copy into <code className="rounded-full bg-muted px-2 py-1 font-mono text-xs text-foreground">components/block/dotted-grid.jsx</code></p>
