...
<ComponentPreview
  component={
    <PreviewVideo showControls={false} src="https://cdn.obsidianui.dev/demos/trading-cards.mp4" label="Trading Card demo" className="w-full h-[450px] rounded-lg" />
  }
  code={`import TradingCard from '@/components/block/trading-card'
...
<Dependencies step={2} title="Copy the source code" source="trading-card">
    <p className="text-[13px] text-muted-foreground mt-2 mb-4">Copy into <code className="text-foreground font-mono text-xs bg-neutral-100 dark:bg-[#161616] px-2 py-1 rounded-md">components/block/trading-card.tsx</code></p>
</Dependencies>
